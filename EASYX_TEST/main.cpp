...
//#include<iostream>
using namespace std;
using atom_t = pair<RECT, string>;
using  text_elmt= vector< pair<atom_t, atom_t>>;	 //first pair is text  second pair is tanc
			...
			if (_kbhit()) //如果是ASCII输入 
			{
				...
				if (c == '\b')
				{
					if (str.length() > 0)
					{
						...
						for (auto& ph : text_obj)
						{
							settextstyle(30, 0, "微软雅黑", 0, 0, 1000, false, false, false);
							if (ph.first.second.find("密码") != string::npos)
							{
								settextstyle(40, 0, "微软雅黑", 0, 0, 1000, false, false, false);
								outtextxy(ph.second.first.left, ph.second.first.top, string(str_v[cur].length(),'*').c_str());
								settextstyle(30, 0, "微软雅黑", 0, 0, 1000, false, false, false);
							}
							else
							{
...
int user_ui()														
{
	...
	for (int i = 0; i < 7; i++)
	{
		...
		s.push_back(pair<atom_t, atom_t>(atom_t(ss, text[i].c_str()), atom_t(ss  , text[i].c_str())));
	}
	vector<string >img;
	img.push_back("atm.bmp");
	vector<string> return_data;

int 	choice=click_interface(s, img, drawing_interface);
	return choice;
...
vector<string> user_login_ui()
{
	...
	int point[2][4] = { { 220,200,380,250 } , { 220,280,380,330 } };
		...
		s.push_back(pair<atom_t, atom_t>(atom_t(ss1, text[0].c_str()), atom_t(ss2, "")));
		ss1 = { point[1][0], point[1][1], point[1][2], point[1][3] };
		ss2= { point[1][0] + 140, point[1][1] + 10, point[1][2] + 70, point[1][3] - 10 };
		s.push_back(pair<atom_t, atom_t>(atom_t(ss1, text[1].c_str()), atom_t(ss2, "")));
...
int query_ui(vector<string> data ) 
{
 ...
 //
	...
	{
		RECT ss = { point[i][0], point[i][1], point[i][2], point[i][3] };
		s.push_back(pair<atom_t, atom_t>(atom_t(ss, text[i].c_str()), atom_t(ss, "1")));
	}
	vector<string >img;
	img.push_back("atm.bmp");
	//vector<string> return_data;
...
vector<string> admin_open_ui()
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (i == 1||i==3)
		{
			...
			s.push_back(pair<atom_t, atom_t>(atom_t(ss, text[i].c_str()), atom_t(ss2, "")));
		}
		else if(i==2)
		{
			RECT ss = { point[i][0], point[i][1], point[i][2], point[i][3] };
...
void test_ui3()
{
	...
	text_elmt s;
	vector<string> text;
	text.push_back("test_ui1");
	text.push_back("test_ui2");
	int point[4][4] = { { 0,200,100,260 } , { 220,180,380,230 }, { 220,260,380,310 }, { 220,340,380,390 } };
	int i = 0;
	RECT ss = { point[i][0], point[i][1], point[i][2], point[i][3] };
	RECT ss1 = { point[1][0], point[1][1], point[1][2], point[1][3] };
	//RECT ss2 = { point[i][0] + 140, point[i][1] + 15, point[i][2] + 240, point[i][3] - 15 };
	s.push_back(pair<atom_t, atom_t>(atom_t(ss, text[i].c_str()), atom_t(ss, "1")));
	s.push_back(pair<atom_t, atom_t>(atom_t(ss1, text[i].c_str()), atom_t(ss1, "1")));
